...
function updateWorldCoinIndex(data, cb) {
    let jsonData = data.Markets.find(findDash => findDash.Name === 'Dash');
        ...
        [   // USD pair
            function (callback) {
                saveMarketData(
                    ...
                    (err, ret) => { error += err; returnData += ret + '\n'; callback(); });
            },
            // EUR pair
            function (callback) {
                saveMarketData(
...
function save(exchangeName, data, cb) {
    ...
    switch (exchangeName.split('market.')[1]) {
        case 'coinmarketcap':
            updateCoinMarketCap(data, (err, ret) => cb(err, ret));
            break;
        case 'worldcoinindex':
            updateWorldCoinIndex(data, (err, ret) => cb(err, ret));
            break;
        default:
